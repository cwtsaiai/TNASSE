...
import counter
from searchspace.nas_201_encoding import BACKBONE
encoding = BACKBONE()
...
class se():
    ...
    def ResourceArrangement(self,s,r):
        ...
        for j in range(self.h):
            for k in range(self.w):
                m[j,k] = np.concatenate((self.get_value(j),encoding.get_rand_code()[self.nfix:]))
    ...
    def Transition(self,s,m):
        ...
        for i in range(self.n):
            for j in range(self.h):
                for k in range(self.w):
                    #first six bits (branch)

                    #random one-point crossover
                    cp = random.randint(self.nfix,self.sl-2-4)
                    c1 = np.concatenate((s[i][:self.nfix],s[i][self.nfix:cp],m[j][k][cp:self.sl-4],s[i][self.sl-4:]))
                    c2 = np.concatenate((s[i][:self.nfix],m[j][k][self.nfix:cp],s[i][cp:self.sl-4],m[j][k][self.sl-4:]))

                    #one-point mutation
                    mp = random.randint(self.nfix,self.sl-1-4) 
                    ...
                    #7-th bit (backbone indicator)
                    num_of_backbone_choices = 4
                    c1[self.sl-4] = np.random.randint(num_of_backbone_choices)
                    c2[self.sl-4] = np.random.randint(num_of_backbone_choices)

                    #8-10 bit (backbone)
                    #random one-point crossover
                    cp = random.randint(self.sl-4+1,self.sl-2)
                    c1 = np.concatenate((c1[:self.sl-4+1],c1[self.sl-4+1:cp],c2[cp:]))
                    c2 = np.concatenate((c2[:self.sl-4+1],c2[self.sl-4+1:cp],c1[cp:]))

                    #one-point mutation
                    mp = random.randint(self.sl-4+1,self.sl-1) 
                    c1[mp] = random.choice(list(set([i for i in range(self.atom)])-set([c1[mp],0]))) #except 0 (none op)
                    c2[mp] = random.choice(list(set([i for i in range(self.atom)])-set([c2[mp],0])))
    ...
    def ExpectedValue(self,v,m,ta,tb):
        ...
        for i in range(self.n):
            for j in range(self.h):
                ...
                s = sum([self.ff(m[j][k]) for k in range(self.w)])
                if s == 0: s = 1e-10
                Mj = self.ff(self.rb[j]) / s
                e[i][j] = Tj * Vij * Mj
        return e
